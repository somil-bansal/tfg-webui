		...
		<div class=" mx-auto inset-x-0 bg-transparent flex justify-center">
			...
			>
				...
				<div class="w-full relative">
					{#if atSelectedModel !== undefined || selectedToolIds.length > 0 || webSearchEnabled || ($settings?.webSearch ?? false) === 'always'}
						<div
								...
								<div class="flex items-center justify-between w-full">
									<div class="pl-[1px] flex items-center gap-2 text-sm dark:text-gray-500">
										<img
											...
											src={$models.find((model) => model.id === atSelectedModel.id)?.info?.meta?.profile_image_url ?? `${WEBUI_BASE_URL}/static/favicon.png`}
										/>
							...
							<div class=" flex justify-between mt-1.5 mb-2.5 mx-0.5 max-w-full">
									...
									<div class="flex gap-0.5 items-center overflow-x-auto scrollbar-none flex-1">
										{#if $_user}
											{#if $config?.features?.enable_web_search && ($_user.role === 'admin' || $_user?.permissions?.features?.web_search)}
												<Tooltip content="Search the internet" placement="top">
													<button
								...
								<div class="self-end flex space-x-1 mr-1 shrink-0">
									{#if toolServers.length > 0}
										<Tooltip content={`${toolServers.length} Available Tool Servers`}>
											<button
