...
# Enable OAuth signup and account merging by email
...
async def oauth_callback(provider: str, request: Request, response: Response):
    ...
    log.info(f"User data received: {user_data}")
    ...
    if not name:
        log.warning(f"OAuth callback failed, name is missing: {user_data}")
        raise HTTPException(400, detail=ERROR_MESSAGES["INVALID_CRED"])
    ...
    jwt_token = create_token(
        data={"id": sub, "email": email, "groups": required_group, "role": role, "name":  name },
        expires_delta=timedelta(hours=8)  # Replace with actual JWT expiry
    )
